...
function initChart() {
    ...
    candleSeries = chart.addCandlestickSeries({
        ...
        priceFormat: {
            type: 'custom',
            formatter: price => adaptiveFormat(price),
            minMove: 0.000001,
        },
    });
    ...
    // ── Адаптивна шкала цін: більше міток при збільшенні ──
    chart.priceScale('right').applyOptions({ autoScale: true });

    // Відстежуємо зміну вертикального масштабу
    let lastPriceRange = null;
    chart.subscribeCrosshairMove(() => {
        updateAdaptivePriceScale();
    });
    // Також оновлюємо при скролі/зумі часової шкали
...
function updateAdaptivePriceScale() {
    if (!candleSeries || !loadedCandles.length) return;
    try {
        const visibleRange = chart.timeScale().getVisibleRange();
        if (!visibleRange) return;

        // Фільтруємо свічки у видимому діапазоні
        const visible = loadedCandles.filter(
        ...
        // Вибираємо minMove залежно від діапазону цін
        let minMove;
        if (range > 100)       minMove = 0.01;
        else if (range > 10)   minMove = 0.001;
        else if (range > 1)    minMove = 0.0001;
        else if (range > 0.1)  minMove = 0.00001;
        else if (range > 0.01) minMove = 0.000001;
        else                   minMove = 0.0000001;

        // Кількість міток: більше при вужчому діапазоні (більший зум)
        // LightweightCharts не дає прямо задати кількість тіків,
        // але через priceFormat.minMove ми контролюємо крок
...
class API:
    ...
    async def _get_exchange(self, exchange_id):
        if exchange_id not in self.exchanges:
            ...
            # Специфічні налаштування для бірж з проблемними ендпоінтами
            options = {'enableRateLimit': True}
            if exchange_id == 'binance':
                # Форсуємо USDT-margined futures (fapi.binance.com) замість dapi
                options['options'] = {
                    'defaultType': 'future',
                    'fetchMarkets': ['linear'],
                }
            elif exchange_id == 'htx':
                # Форсуємо лінійні свопи (api.huobi.pro) замість hbdm
                options['options'] = {
                    'defaultType': 'swap',
                    'fetchMarkets': ['linear'],
                }
    ...
    async def _fetch_markets_from_exchange(self, exchange_id):
        ...
        for sym, m in raw.items():
            # Спочатку перевіряємо булеві поля ccxt
            if m.get('swap') or m.get('linear') or m.get('inverse'):
                ...
                # Fallback на текстове поле 'type' (BitMEX та ін.)
